    ...
    public class Tests
    {
        ...
        public void test_page1()
        {
            ...
            for (int a = 0; a < 10; a++)
            {
                ...
                {
                    try { sText[i].Click(); sText[i].SendKeys("Name"); } catch (Exception) { }
                }
                var sTextArea = driver.FindElements(By.XPath("//textarea"));
                for (int i = 0; i < sTextArea.Count; i++)
                {
                    try { sTextArea[i].Click(); sTextArea[i].SendKeys("Name person"); } catch (Exception) { }
                }
                ...
                var sEmail = driver.FindElement(By.XPath("//input[@type='email']"));
                try { sEmail.Click(); sEmail.SendKeys("aleksandr.ivanov.230600@gmail.com"); } catch (Exception) { }
            ...
            try { sText1.Click(); sText1.SendKeys("aleksandr.ivanov.230600@gmail.com"); } catch (Exception) { }
            var sText2 = driver.FindElement(By.XPath("//input[@type='password']"));
            try { sText2.Click(); sText2.SendKeys("55s95a87n89ja"); } catch (Exception) { }
            Thread.Sleep(100);
